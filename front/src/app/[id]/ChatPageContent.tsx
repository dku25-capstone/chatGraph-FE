  ...
  useEffect(() => {
    ...
    if (isOptimistic) {
      ...
      if (optimisticDataString) {
        ...
        console.log(
          "-> Found prompt in sessionStorage, creating fake response."
        );
        ...
        askQuestion({ questionText: prompt })
          .then((realResponse) => {
            console.log(
              "-> askQuestion SUCCESS. Saving to store and replacing URL."
            );
            const topicNode = realResponse.nodes[
              realResponse.topic
            ] as TopicNode;
        ...
        console.log(
          "-> Prompt NOT found in sessionStorage, redirecting to home."
        );
        router.replace("/");
      }
